...
//Checks if the visiting IP is on the whitelist
include '/var/www/xsceditor/secure/DewUtils.php';
logvisit();
...
if($_POST['file_to_delete'] != ''){
	$to_delete = $_SERVER['DOCUMENT_ROOT'] . "/xsceditor/xscoutput" . $_POST['file_to_delete'];
	unlink($to_delete);
	echo "$to_delete deleted";
}

$dir = $_SERVER['DOCUMENT_ROOT'] . "/xsceditor/xscoutput/";
...
echo <<<EOT
	...
	<h1><i>XSC Creations Manager</i></h1>
...
foreach($files as $file){
	...
	<input type="hidden" name="uploadedfilename" value="$file">
	<input type="hidden" name="uploadsoroutput" value="output">
